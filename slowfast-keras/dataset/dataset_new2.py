...
def get_ucf101(video_path, file_path, name_path, mode, num_classes):
    ...
    for path_label in open(file_path, 'r'):
        #mbashat
        print('path_label_split:', path_label.split())
        if mode == 'train':
...
class DataGenerator(Sequence):
    ...
    def data_generator(self, batch_dataset):
        #print('mbashat: inside data_generator')
        video_data = []
        label_data = []
        for data in batch_dataset:
            ...
            clip = load_clip_video(path, new_frame_indices)
            #intervals = int(math.ceil(length/frames_per_interval))
